...
/*
 * Copyright (c) 2019-2021. Taimos GmbH http://www.taimos.de
 */
...
function getConfigObject() : ConfigurationServicePlaceholders & APIVersions {
  return {
...
async function showAccounts() : Promise<void> {
  ...
  if (args.tags && args.tags.length > 0) {
    for (const acc of accounts) {
      ...
      tags.set(acc.Id!, new Map());
      res.Tags!.forEach((tag) => {
        tags.get(acc.Id!)?.set(tag.Key, tag.Value);
      });
    }
  }
  ...
  accounts.sort((a, b) => a.Id!.localeCompare(b.Id!)).forEach((acc) => {
    ...
    const tagList = (args.tags && args.tags.length > 0) ? ',' + args.tags.split(',').map((key : string) => tags.get(acc.Id!)?.get(key)).join(',') : '';
